...
quietly set FPU_Testcases [list fpu_test_simple_sanity fpu_test_sequence_fair fpu_test_neg_sqr_sequence fpu_test_random_sequence]
quietly set Tests  3
...
foreach {Testcase} $FPU_Testcases {
  ...
  for {set N 0} {$N<$Tests} {incr N} {
    ...
    vsim  -sv_seed $N	-onfinish stop -wlfnocollapse -assertdebug -msgmode both -coverage -displaymsgmode both -quiet +UVM_TESTNAME=$Testcase +UVM_VERBOSITY=UVM_MEDIUM  optimized
    #vsim  -sv_seed random	-onfinish stop -wlfnocollapse -assertdebug -msgmode both -coverage -displaymsgmode both -quiet +UVM_TESTNAME=$Testcase +UVM_VERBOSITY=UVM_MEDIUM  optimized
