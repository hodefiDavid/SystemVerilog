...
vlog -f vlog_sv.f
vopt -quiet -L dut_gate_lib +nocheckALL top -o top_opt -sdfmin /top/fpu_dut=../../fpu/netlist/verilog/fpu_flat.sdf
vsim +UVM_TESTNAME=fpu_test_random_sequence -onfinish stop -c top_opt -nosva  -sv_seed 1 +count=300
run -all
quit -sim

# execute run 2 with seed = 10
vsim +UVM_TESTNAME=fpu_test_random_sequence -onfinish stop -c top_opt -nosva  -sv_seed 10 +count=300
run -all
quit -sim

# execute run 3 with seed = 100
vsim +UVM_TESTNAME=fpu_test_random_sequence -onfinish stop -c top_opt -nosva  -sv_seed 100 +count=300
run -all
quit -sim
